...
angle = 0
Lrot = 0
def c_Bezier(q, qdot, dt, solo, t_simu, key,trot):
    # unactuated, [x, y, z] position of the base + [x, y, z, w] orientation of the base (stored as a quaternion)
    # qu = q[:7]
    # actuated, [q1, q2, ..., q8] angular position of the 8 motors
    qa = q[7:]
    ...
    global qa_dot2_ref, q_dot2_ref, q_dot_ref, q_ref, flag_q_ref, T, dx, dz, V, angle, Lrot
    ...
    #Initialization of the variables
    K = 100.  #Convergence gain
    T = 3  #period of the foot trajectory
    xF0 = 0.19  #initial position of the front feet
    xH0 = -0.19  #initial position of the hind feet
    yR0 = -0.147
    yL0 = 0.147
    ...
    # Desired foot trajectory
    xzdes_FL = bodytoFeet[1,:].reshape((3,1))
    xzdes_HR = bodytoFeet[2,:].reshape((3,1))
    xzdes_FR = bodytoFeet[0,:].reshape((3,1))
    xzdes_HL = bodytoFeet[3,:].reshape((3,1))
