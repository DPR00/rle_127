...
class trotGait:
    ...
    def calculateBezier_swing(self , phi_sw , V , angle,T):#phi between [0,1), angle in degrees
...
##            print('foot UP', self.s , phi)
        ...
        swingY = 0.
        swingZ = 0.
        for i in range(12): #sum all terms of the curve
    ...
    def stepTrajectory(self , phi , V , angle , Wrot , centerToFoot,T): #phi belong [0,1), angles in degrees
        if (phi >= 1):
            phi = phi - 1.
...
#            print(phi,phiStance, stepX_long)
            ...
            stepX_long , stepY_long , stepZ_long = self.calculateBezier_swing(phiSwing , V , angle,T)#longitudinal step
            stepX_rot , stepY_rot , stepZ_rot = self.calculateBezier_swing(phiSwing , Wrot , circleTrayectory,T)#rotational step
...
#        print(self.phi)
        ...
        step_coord = self.stepTrajectory(self.phi + offset[0] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[0,:])),T) #FR
        ...
        step_coord = self.stepTrajectory(self.phi + offset[1] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[1,:])),T)#FL
        ...
        step_coord = self.stepTrajectory(self.phi + offset[2] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[2,:])),T)#BR
        ...
        step_coord = self.stepTrajectory(self.phi + offset[3] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[3,:])),T)#BL
